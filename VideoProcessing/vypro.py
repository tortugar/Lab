...
def intan_correct_videotiming(ppath, rec, fr=1, pplot=True):
    ...
    :param fr: frame rate of time scale used for annotation; Say, you want to annotate each 0.5s timestep,
               then set fr = 2
    ...
    The function saves two arrays in the file os.path.join(ppath, rec, video_timing.mat):
        'onset' - the time point of the onset of each video frame
        'tick_onset' - a time scale determining which video frame will be annotated
...
def intan_correct_videotiming_highfr(ppath, rec, set_cam_fr, fr=1, pplot=True):
    """
    set annotation time scale for videos from intan.
    The function assumes that the following two files exist (which are generated by data_processing.py):
    timestamp_"rec".mat and videotime_"rec".mat
    :param ppath: base folder
    :param rec: recording name
    :param set_cam_fr: camera frame rate set on computer (which may differ from the real frame rate)
    :param fr: frame rate of time scale used for annotation; Say, you want to annotate each 0.5s timestep,
               then set fr = 2
    :param pplot: if True, plot distance between frames; for debugging purposes
    :return: onset, np.array; array with time points of each frame onset
    ...
    print('Camera time steps: ' + str(cam_sr))
    ...
    for i in range(0, len(tstamp)-1):
        if cam_sr[i] > 1.5 * cam_dt:
            # camera dropped frame
            if d[i] > 2*cam_dt+cam_dt/5:
                # frame dropped
                n = np.round((d[i]-cam_dt) / cam_dt)
                curr_onset_idx += int(n)
                ndropped += int(n)-1
                onset_idx.append(curr_onset_idx)
                idx_dropped.append(i)
...
def opto_videoseq(ppath, name, tsegs, nidle=5, fmax=20, emg_legend=1000,
                    vm=2.0, time_legend=10, ffmpeg_path='ffmpeg', titles=[], color_map='jet', filt_dim=(), pphasic=False):
    """
    ...
    :param filt_dim: tuple, defines dimension of a box filter for smoothing the EEG spectrogram;
                     ...
                     result in a smoother frequency axis. Previously, I used filt_dim = (2,4).
                     If filt_dim = (), no filtering operation is performed
    :param pphasic: bool, if True, indicate the occurence of phasic REM using a little arrow
    ...
    ###########################################################################
    # get phasic REM events
    event_list = []
    if pphasic:
        phasic_rem = sleepy.phasic_rem(ppath, name)
