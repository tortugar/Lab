...
def spindle_correlation(ppath, unit_listing, pre, post, xdt = 0.1, backup='', std_thr=1.5, 
                        pzscore=False, ma_thr=10, ma_rem_exception=False, pplot_spindles=False):
...
def phrem_correlation(ppath, unit_listing, pre, post, xdt=0.1, pzscore=False, backup='', 
                      ma_thr=10, ma_rem_exception=False, pplot_phrem=False):
    """
    Correlate firing rates with phrem REM (phasic theta events) in the EEG
    ...
    ----------
    ppath : str
        basee folder with recordings
    unit_listing : str or list
        unit listing, either text file, or dict as returned 
        by &load_units() (each value of a list of Recording objects, see class Recording)
    pre : float
        time interval before phasic REM onset.
    post : float
        time interval after phasic REM onset.
    xdt : TYPE, optional
    ...
    ma_rem_exception : TYPE, optional
        DESCRIPTION. The default is False.
    pplot_phrem : bool, optional
        If True, plot figure as generated by &sleepy.phrem_rem(). The default is False.
    ...
    for k in units:
        for unit in units[k]:
            # units[k] is an ID
            # unit.name is the recording name
    ...
    for (name, path) in zip(recordings, paths):
        ...
        # dict: unit_ID :---> units objects in the current recordings $name;
        # why? The the same unit_ID may also be associated with other recordings
        units_in_name = {}
        # go through all units in the current recording:
        for unit_ID in rec_units:
            # go through all recordings where this unit comes up:
            for unit in units[unit_ID]:
                # ... and only take the unit instance in the currently considered recording $name
                if unit.name == name:
                    if unit_ID in units_in_name:
                        units_in_name[unit_ID].append(unit)
                    else:
        ...
        phrem_in_name = sleepy.phasic_rem(path, name, pplot=pplot_phrem)
        
        # go through all unit IDs for the given recording $name
        for unit_ID in units_in_name:
            units_for_ID = units_in_name[unit_ID]
